...
// `viewClickProvider`.
async function generateViewsAndClicks(test, uuid, viewClickProvider, igOwner,
                                      numViews, produceAttributionSrc,
                                      produceUserAction) {
...
async function testClickiness(test, igOwner, viewClickProvider, numViews,
                              produceAttributionSrc, produceUserAction,
                              viewAndClickCountsProviders = undefined) {
  const uuid = generateUuid(test);

  await generateViewsAndClicks(test, uuid, viewClickProvider, igOwner,
                               numViews, produceAttributionSrc,
                               produceUserAction);
  ...
  // `viewClickProvider`s.
  let clicksBadTest = produceAttributionSrc && produceUserAction ?
    "< 1" : " !== 0";

  let viewsBadTest = (numViews > 0) ?
    `< ${numViews}` : " !== 0";
  ...
  await joinCrossOriginInterestGroup(test, uuid, igOwner,
                                     interestGroupOverrides);

  const auctionConfigOverrides = {
    interestGroupBuyers: [ igOwner ]
  };

  // Run auctions until we see the recorded events in the browserSignals.
  while (true) {
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(test, IG_OWNER, VIEW_CLICK_PROVIDER, /*numViews=*/2,
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(test, IG_OWNER, VIEW_CLICK_PROVIDER, /*numViews=*/4,
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(test, IG_OWNER, VIEW_CLICK_PROVIDER, /*numViews=*/6,
                       /*produceAttributionSrc=*/true,
                       /*produceUserAction=*/true,
                       []);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(test, IG_OWNER, VIEW_CLICK_PROVIDER, /*numViews=*/0,
                       /*produceAttributionSrc=*/true,
                       /*produceUserAction=*/true, []);
...
subsetTest(promise_test, async test => {
  ...
  await testClickiness(test, IG_OWNER, VIEW_CLICK_PROVIDER, /*numViews=*/1,
                       /*produceAttributionSrc=*/true,
...
subsetTest(promise_test, async test => {
  ...
  // From provider 1 have views, no clicks;
  // From provider 2 have click, no views.
  await generateViewsAndClicks(test, uuid, VIEW_CLICK_PROVIDER1, IG_OWNER,
                               /*numViews=*/2, /*produceAttributionSrc=*/false,
                               /*produceUserAction=*/false);
  await generateViewsAndClicks(test, uuid, VIEW_CLICK_PROVIDER2, IG_OWNER,
  ...
  // Run auctions until we see the expected recorded events in the
  // browserSignals.
  while (true) {
...
}, 'browserSignals for clickiness --- unrequested providers not included.');
